...
describe("PATCH /api/v1/activations/[token_id]", () => {
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        "http://localhost:3000/api/v1/activations/8f4d62cd-1791-4fde-a1fe-7910aae33529",
        {
          method: "PATCH",
        },
      );
  ...
  test("With valid token but user already activated user", async () => {
    const createdUser = await orchestrator.createUser();
  ...
  describe("Default user", () => {
    test("With valid token, but already logged in user", async () => {
      ...
      expect(responseBody).toEqual({
        ...
        action:
          'Verifique se sua conta possui a feature "read:activation_token" habilitada.',
        status_code: 403,
      });
    });
  });
